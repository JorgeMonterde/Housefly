...
namespace Housefly.Construction
{
    public class HingedPiecesComponent : GH_Component
    {
        ...
        {
            // IMPROVEMENTS: 
            // - replace guide plane with a simple angle to rotate the hinges plane around the rail direction
            // - option to avoid rectangle construction; we may want the hinged behaviour but on custom pieces that should be oriented into (only interested in planes)
